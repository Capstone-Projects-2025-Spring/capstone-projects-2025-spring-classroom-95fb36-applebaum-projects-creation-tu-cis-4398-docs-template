...
from .llm import explainConcept, generate_comment_suggestion, generate_suggestion, generate_incorrect_suggestion
from .models import CodeRequest, CommentRequest, ExplanationRequest, ExplanationResponse, SuggestionResponse
...
@router.post('/commentSuggest', status_code=200, response_model=SuggestionResponse)
async def get_comment_suggestion(request: CommentRequest) -> dict:
# async def get_suggestion() -> dict:
    ...
    **Response:**
    ...
    try:
        ...
        #     instructions="Complete the code. Do not add any comments and keep function as concise as possible.",
        # )
        generated_code = await generate_comment_suggestion(request)
        print("comment suggestion created")
        return {"Response": generated_code}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))